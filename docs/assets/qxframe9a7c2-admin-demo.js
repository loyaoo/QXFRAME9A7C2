(function(){
  ...
  function initListControls(){
    ...
    if(typeHost)keep(C.Select.create({container:typeHost,value:'all',items:['all','专题页','文章','商品','单页','图集','下载'].map(function(v){return{key:v,value:v,label:v==='all'?'全部类型':v};}),clearable:false,searchable:true,onChange:function(v){listState.type=v||'all';listState.page=1;syncList();}}));
    var statusHost=byId('admin-list-status-select');
    if(statusHost)keep(C.Select.create({container:statusHost,value:'all',items:['all','已发布','草稿','审核中','已下线'].map(function(v){return{key:v,value:v,label:v==='all'?'全部状态':v};}),clearable:false,onChange:function(v){listState.status=v||'all';listState.page=1;syncList();}}));
    var dateHost=byId('admin-list-date-picker');
    if(dateHost)keep(C.DatePicker.create({container:dateHost,value:null,clearable:true,placeholder:'选择日期',onChange:function(v){listState.date=v&&v.getFullYear?String(v.getFullYear())+'-'+String(v.getMonth()+1).padStart(2,'0')+'-'+String(v.getDate()).padStart(2,'0'):'';listState.page=1;syncList();}}));
  ...
  function initFormControls(){
    ...
    var typeHost=byId('admin-content-type-select'); if(typeHost)keep(C.Select.create({container:typeHost,name:'type',value:'topic',searchable:true,clearable:true,items:[{key:'topic',value:'topic',label:'专题页'},{key:'article',value:'article',label:'文章'},{key:'product',value:'product',label:'商品'},{key:'page',value:'page',label:'单页'}]}));
    var catHost=byId('admin-category-cascader'); if(catHost)keep(C.Cascader.create({container:catHost,name:'category',value:'season',clearable:true,items:[{key:'marketing',value:'marketing',label:'市场活动',items:[{key:'season',value:'season',label:'季节专题'},{key:'campaign',value:'campaign',label:'营销活动'}]},{key:'product',value:'product',label:'商品中心',items:[{key:'table',value:'table',label:'茶桌'},{key:'chair',value:'chair',label:'茶椅'}]}]}));
    var tags=byId('admin-tags-input'); if(tags)keep(C.TagInput.create({container:tags,name:'tags',value:[{key:'autumn',value:'autumn',label:'秋季新品'},{key:'topic',value:'topic',label:'专题'}],creatable:true,tokenSeparators:[',',';'],placeholder:'输入标签后回车'}));
    var date=byId('admin-publish-date'); if(date)keep(C.DatePicker.create({container:date,name:'publishAt',value:'2026-09-19',clearable:true,placeholder:'选择发布日期'}));
    ...
    var upload=byId('admin-cover-upload'); if(upload)keep(C.Upload.create({container:upload,name:'cover',multiple:false,drag:true,autoUpload:false,listType:'text',accept:'image/*,video/*',beforeUpload:function(){return false;}}));
  }
