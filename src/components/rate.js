...
export class Rate extends FieldComponent {
    ...
    #setCommitted(next, config={}) {
        ...
        if((config.user===true||config.request===true)&&r.valueState.controlled)r.valueState.requestChange(normalized,{silent:true,reason:config.reason||'set-value',source:config.source||'api',originalEvent:config.originalEvent||null});
    ...
    [componentHooks.render]() {
        ...
        r.formBridge=this.own(Control.createFormFieldBridge({root,target:opts.container,formField:opts.formField,document:doc,moveIntoRoot:false,projectLayout:Control.projectFormFieldLayout,name:opts.name,disabled:opts.disabled===true,readOnly:opts.readOnly===true,required:opts.required===true,value:r.valueState.value,serializeValue:opts.serializeValue,getValue:()=>r.valueState.value,onReset:()=>{if(!r.valueState.controlled)this.#setCommitted(r.initialValue,{silent:true,source:'form',reason:'reset'});else{r.hoverValue=0;this.setFieldValue(r.valueState.value,{silent:true,force:true,sync:true,source:'form',reason:'reset-preserve'});this.#renderValue();if(r.formBridge)r.formBridge.setValue(r.valueState.value,{silent:true});}}}));
