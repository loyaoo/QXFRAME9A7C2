...
function createRuntime(instance, prepared) {
    ...
    function setValues(next, config) {
        ...
        const changed=!sameValues(values,normalized);if(changed){const stateMeta={silent:true,source:settings.source||(settings.user===true?'user':'api'),reason:settings.reason||'set-value',originalEvent:settings.originalEvent||null};if(settings.user===true&&valueState.controlled)valueState.requestChange(normalized,stateMeta);else{valueState.setValue(normalized,stateMeta);values=valueState.value;instance.setFieldValue(externalValue(),{force:true,silent:true,reason:stateMeta.reason});}}
    ...
    values=valueState.value;instance.setFieldValue(externalValue(),{force:true,silent:true,reason:'slider-init'});
