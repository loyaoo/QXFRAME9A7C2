...
function createHierarchy(options) {
  ...
  function stateForLeaves(leaves, selected) { var set = selected instanceof Set ? selected : new Set((selected || []).map(String)), count=0; for(var index=0;index<leaves.length;index+=1)if(set.has(leaves[index]))count+=1; return Object.freeze({ checked: leaves.length>0 && count===leaves.length, indeterminate: count>0 && count<leaves.length, selectedCount:count, total:leaves.length, keys:leaves }); }
  function stateFor(item, selected) { return stateForLeaves(leafKeys(item), selected); }
  function toggle(item, selected, checked) { var set = selected instanceof Set ? new Set(selected) : new Set((selected || []).map(String)), leaves=leafKeys(item), state=stateForLeaves(leaves,set), next = checked === undefined ? !state.checked : checked === true; leaves.forEach(function(key){ if(next) set.add(key); else set.delete(key); }); return Array.from(set); }
