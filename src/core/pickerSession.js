...
function createPickerSession(options) {
  ...
  var controller = opts.controller || ValueController.create({
    value: opts.value,
    controlled: opts.controlled === true,
    normalizeValue: opts.normalizeValue || opts.normalize,
    copyValue: opts.copyValue || opts.copy || ValueEquality.copy,
  ...
  function close(detail) {
    ...
    if (controller.dirty && shouldRollback(info)) {
      rolledBack = cancel({ silent:true, source:'popup', reason:info.reason || 'close', originalEvent:info.originalEvent || null }) !== false;
    }
    if (Utils.isFunction(controller.endSession)) controller.endSession({ silent:true, source:info.source || 'popup', reason:info.reason || 'close', originalEvent:info.originalEvent || null });
    info.committed = false;
    info.commitRejected = false;
