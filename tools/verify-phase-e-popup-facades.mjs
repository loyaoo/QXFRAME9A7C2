...
const modal=Modal.create({document,portalContainer:document.body,autoOpen:false,duration:0,maskDuration:0,content:'M'});a(modal.getOverlayFamilyController()===modal.getOverlayController(),'modal legacy alias');a(!!modal.getOverlayResourceController()&&modal.getOverlayResourceController().getRuntime()===modal.getOverlayRuntime(),'modal migrated resource controller/runtime identity');modal.open('verify');a(modal.getState().open===true,'modal family logical controller still owns open');modal.close('verify');modal.destroy();
const drawer=Drawer.create({document,portalContainer:document.body,autoOpen:false,duration:0,animation:false,content:'D'});a(drawer.getOverlayFamilyController()===drawer.getOverlayController(),'drawer legacy alias');a(!!drawer.getOverlayResourceController()&&drawer.getOverlayResourceController().getRuntime()===drawer.getOverlayRuntime(),'drawer migrated resource controller/runtime identity');drawer.destroy();
